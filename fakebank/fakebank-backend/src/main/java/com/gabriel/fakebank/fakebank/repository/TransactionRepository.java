...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.cpf = :cpf AND t.bank = :bank AND t.category = 'invoice' AND t.paid = true")
    ...
    @Query("SELECT t FROM Transaction t WHERE t.cpf = :cpf AND t.bank = :bank AND t.date BETWEEN :start AND :end")
    List<Transaction> findAllByCpfAndBankAndDateBetween(
            @Param("cpf") String cpf,
            @Param("bank") Bank bank,
            @Param("start") LocalDate start,
            @Param("end") LocalDate end
